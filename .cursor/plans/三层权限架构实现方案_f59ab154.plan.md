...
3. **底层（熔断层）**：EMERGENCY_ADMIN_ROLE，单签或1-of-3，无延迟
...
flowchart TB
    ...
    subgraph bottomLayer [底层: 熔断层]
        BottomMultisig[Gnosis Safe<br/>1-of-3 多签<br/>或单签]
...
**文件**: `src/contracts/protocol/configuration/MultisigRoleAdapter.sol`
...
**文件**: `src/contracts/dependencies/openzeppelin/contracts/governance/ITimelockController.sol`
...
**文件**: `src/contracts/protocol/configuration/TimelockRoleManager.sol`
...
**文件**: `src/contracts/protocol/configuration/ACLManager.sol`
...
**文件**: `src/contracts/protocol/pool/PoolConfigurator.sol`
...
**文件**: `src/deployments/scripts/DeployMultisigAccessControl.s.sol`
...
**控制实体**：专用硬件钱包/自动化风控系统（1-of-3或单签）

- 持有者：银行首席风控官(CRO)或7x24小时监控中心
- 执行门槛：1票
...
1. **向后兼容**：保持现有接口不变，确保现有代码正常工作
2. **Gas成本**：多签验证已优化，仅检查地址匹配
3. **审计要求**：多签集成是安全关键点，需要专业审计
4. **文档更新**：更新相关文档说明新的权限架构
5. **紧急情况**：底层熔断层可立即暂停协议，但无法修改配置
6. **升级路径**：如需升级，必须通过顶层多签钱包和Timelock流程